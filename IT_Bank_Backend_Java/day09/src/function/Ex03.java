...
public class Ex03 {
	...
	static int getFee(int time) {
		...
		if (time > 30) {					// 중간에 제어문 및 연산자, 변수등으로
	...
	// 4) 함수 정의에서는 함수의 구체적인 실행 내용을 작성해야함으로 ; 대신 {}를 열어줌.
	// 5) 자바의 함수는 반환자료형의 값을 반드시 반환해야만한다.
	// 따라서, 첫 줄에는 반환형 타입의 변수를 선언 및 초기화하고 마지막줄에 return하도록 한다.
	// 6. 첫줄과 마지막줄이 만들어졌다면 그 사이에서 매개변수 값을 활용한 코드를 작성해야함.
