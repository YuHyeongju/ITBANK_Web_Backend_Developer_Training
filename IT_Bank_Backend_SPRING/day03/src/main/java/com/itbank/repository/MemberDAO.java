...
public interface MemberDAO {
	...
	@Update("update member set userpw = #{userpw}, username=#{username}, gender=#{gender}"
			+ " where userid = #{userid} and idx=#{idx}")
	int update(MemberDTO dto);

	@Update("update member set userpw = #{userpw} where userid = #{userid} and email = #{email}")
	int updatePassword(MemberDTO dto);
	
	// 마이바티스에서 매개변수는 하나이거나  없어야한다.
