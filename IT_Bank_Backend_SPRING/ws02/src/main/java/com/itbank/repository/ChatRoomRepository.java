...
public class ChatRoomRepository {
	...
	public List<RoomDTO> finAllRooms() {	// 모든 방의 객체를 리스트로 반환
		List<RoomDTO> result = new ArrayList<>(roomMap.values()); // Map의 values만 추출
		Collections.reverse(result);							  // 순서 뒤집기(최신방 먼저)
		return result;
	}
	
	public RoomDTO findRoomById(String id) {  // 저장된 방은 각각 고유의 id가 있다.
		return roomMap.get(id); 				// id를 key로 사용하여 방을 찾아 반환
	}
	
	public RoomDTO createChatRoom(String name) { // 방 생성, 이름을 전달 받는다.
