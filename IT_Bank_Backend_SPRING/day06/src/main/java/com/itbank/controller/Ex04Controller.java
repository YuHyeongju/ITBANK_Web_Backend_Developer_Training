...
public class Ex04Controller {
	@Autowired private Ex04Service es;
	
	@GetMapping
	public ModelAndView ex04() {
		ModelAndView mav = new ModelAndView();
		List<Upload2DTO> list = es.getList();
	...
	public String ex04(Upload2DTO dto) {
		...
		//dto(커맨드 자체)내부에 필드가 List타입이라면
		// 같은 이름의 여러 값을 리스트 형태로 자동을 받는다.
		// 이런 형식을 중첩 커맨드 객체라고 한다.
		// 중첩 커맨드 객체는 checkBox를 List<String> 으로 받을 때도 사용할 수 있다.
		// 단, DTO없이 (커맨드 객체를 사용하지 않고) 매개변수에 LIST를 지정하면 처리할 수 없음.
		// 체크박스와 파일은 여러개가 한번에 들어올 수 있음.
