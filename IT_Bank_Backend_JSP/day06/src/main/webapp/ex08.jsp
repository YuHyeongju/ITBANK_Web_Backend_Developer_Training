...
<fieldset>
	<legend>c:if</legend>
	<h3>test의 조건이 true이면 태그 내부 코드를 수행한다.</h3>
	<p>
		${t2 }의 나이는  ${t3 }살이고
...
<fieldset>
	...
	<h3>여러 조건중 하나를 수행한다. 하위 태그로 c:when, c:otherwise가 있다.</h3>
	<c:choose>
...
<fieldset>
	...
	<c:forTokens var="word" items="${text }" delims=" ">
		<p>${word }</p>
	</c:forTokens> <%-- split이 내장되어있다. --%>
</fieldset>

<fieldset>
	<legend>c:redirect</legend>
	<h3>지정한 페이지로 리다이렉트한다.</h3>
	&lt;c:redirect url="https://www.naver.com"/> <%-- 태그를 써버리면 진짜 네이버로 리다이렉트함 --%>
