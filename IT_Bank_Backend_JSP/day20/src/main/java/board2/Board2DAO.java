...
public class Board2DAO {
	...
	//게시글 갯수 불러오기
	public int selectCount1(String search) {
		int count = 0;
	...
	//내가 쓴 글의 게시글 갯수 불러오기
	public int selectCount2(String userid) {
		int count = 0;
		
		String sql = "select count(*) from board2 where writer = ?";		
		try {
			conn =ds.getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	public List<ReplyDTO> selectReplyList(int idx){
		...
		String sql = "select * from reply2 where board_idx = ?"
