...
<main>
	...
	<details open class="frame" style="margin:0 auto; padding-top:10px;">
		...
		<c:set var="boardCount"
			value="${board2DAO.selectCount(param.search) }" />
		<c:set var="paramPage" value="${empty param.page? 1: param.page }" />
		<c:set var="paging"
			value="${Paging.newInstance(paramPage, boardCount) }" />
		<c:set var="list"
			value="${board2DAO.selectList(param.search, paging) }" />
		<ul 
