...
<body>
	<h1>사용자의 입력값을 리스트에 추가하고 화면에 반환하기</h1>
	<hr>
	...
	<form>
		...
		<p>
			<input type="text" name="storeName" placeholder="식당이름">
		</p>
		<p>
			<input type="text" name="menuName" placeholder="메뉴이름">
		</p>
		<p>
			<input type="number" name="menuPrice" placeholder="주문금액" min="0" step="100" required>
		</p>
		<p>
			<input type="submit">
		</p>
	...
	<%--ArrayList 클래스를 사용하기 위해 import가 필요하기 때문에 지시자(directive)를 사용한다. --%>
	
	<%!// 선언부(declaration, 멤버필드 및 멤버 메서드를 선언/정의한다.)
		private ArrayList<FoodDTO> list = new ArrayList<>();%>
		<%--%!가 없어지면 값을 유지할 수 없기때문에(지역변수 처리됨) 입력할때 마다 값이 초기화된다. --%>
	...
	<% // 스크립틀릿 내부 코드는 service() 함수 내부 내용이다. 
	   ...
	   // 4개의 파라미터 전달받기
		...
		if(category != null){	// 파라미터가 있기때문에 새로고침을 하면 똑같은 값을 계속 추가하게된다.  
