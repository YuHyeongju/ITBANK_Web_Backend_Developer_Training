...
<h3>서버에서 특정값을 문자열로 생성하여 클라이언트에 보관하는 형식의 데이터</h3>
<h3>문자열 형태로 저장해두면, 다음번 방문 시 서버가 값을 회수하여 활용할 수 있음.</h3>
...
<fieldset>
	...
	<%
		// 클라이언트가 서버에 방문 시 가지고있던 쿠키를 요청에 담아서 보낸다.
		// 첫번째 방문이라면 cookies 배열의 값은  null일 수도 있음.
		Cookie[] cookies = request.getCookies();
	
		 String value = ""; // 쿠키는 항상 문자열 형태이다. 쿠키의 값을 저장할 변수를 미리 만들어둔다.
		 
		 
		 //cookies가 null이면 cookies.length에서 NullPointerException이 발생할 수 있음.
