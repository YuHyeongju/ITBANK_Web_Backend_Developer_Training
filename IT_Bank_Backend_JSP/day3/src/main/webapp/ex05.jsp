...
<body>
	...
	<h4>parameter</h4>
	<%--서버에서 클라이언트에게 값을 보내줌, 그 값 = 파라미터 --%>
	<ul>
		<li>파라미터는 사용자의 입력값을 말한다.</li>
		<li>쿼리스트링 형태로 있을수 있으며 문자열로 구성된다.</li>
		<li>HTTP Method가 POST인 경우, 문자열 혹은 파일 객체로 구성될 수 있다.</li>
		<li>사용자의 입력값이므로, 서버 개발자가 값을 강제로 넣을 수 없다.(setParameter는 없다.)</li>
	...
	<ul>
		...
		<li>JSP 내장 객체에 저장해 둘 수 있고, 내장객체에 따라 유효범위가 다르다.</li>
		<li>서로 다른 JSP페이지가 attribute를 공유할 수 있다.</li>
		<li>서버 개발자가 값을 set 혹은 get 할 수 있다.</li>
		<li>Object타입을 원하는 형태로 바꾸려면, 형변환(다운캐스팅)을 해야함.</li>
		<%-- FoodDTO ob = (FoodDTO) request.getAttribute("food") --%>
	</ul>
	...
	request.setAttribute("test1", test1); // 꺼내거나 지울때 필요한 이름
	request.setAttribute("test2", test2);
	request.setAttribute("test3", test3);
	request.setAttribute("test4", test4);

	String ob1 = (String) request.getAttribute("test1"); // 값을 꺼낼때 다운캐스팅을 해줘야한다.
	FoodDTO ob2 = (FoodDTO) request.getAttribute("test2");
	Integer[] ob3 = (Integer[]) request.getAttribute("test3");
	List<Integer> ob4 = (List<Integer>) request.getAttribute("test4"); // Wrapper클래스 까지 Integer인지 모르겠다.
	...
	<table border="1" cellpadding="10" cellspacing="0">
		...
		<tr>
			<td>request</td>
			<%--페이지당 하나 --%>
			<td>현재 요청에 대해서만 사용할 attrubute를 저장한다.<br> forword를 이용하면,
				forword 대상 페이지에서도 attribute를 참조할 수 있다.
			</td>
		</tr>
		<tr>
			<td>session</td>
